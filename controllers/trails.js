...
exports.getTrailData = async (req, res) => {
  try {
    ...
    const {
      ...
      `http://washcotrails.flitchbeta.com/wp-json/wp/v2/trails?slug=${trailHandle}`
    );
...
exports.getTrails = async (req, res) => {
  try {
    const { data: trails } = await axios.get(
      'http://washcotrails.flitchbeta.com/wp-json/wp/v2/trails?per_page=500&order=asc&fields=title,slug,custom_data,regions'
    );
...
exports.getRegionData = async (req, res) => {
  try {
    const { data: regions } = await axios.get(
      'http://washcotrails.flitchbeta.com/wp-json/wp/v2/regions'
    );
    const regionsArray = regions.map(region => {
      const lat = region.custom_data.markerCoordinates
        ? Number(region.custom_data.markerCoordinates.marker_latitude)
        : 0;
      const lng = region.custom_data.markerCoordinates
        ? Number(region.custom_data.markerCoordinates.marker_longitude)
        : 0;
      return {
