...
export const enhanceTrail = async (trail, updateConnectorTrailCoords, updateTrailCoords) => {
  ...
  if (!enhancedTrail.connector_coordinates) {
    ...
    if (
      ...
      try {
        const {data: { trail: { coordinates: coords } } } = await axios.get(`/api/coordinates?url=${enhancedTrail.custom_data.connectorTrailJSON.url}`)
  ...
  if (!enhancedTrail.coordinates) {
    ...
    if (
      ...
      try {
        const {data: { trail: { coordinates: coords } } } = await axios.get(`/api/coordinates?url=${enhancedTrail.custom_data.jsonCoordinates.url}`)
